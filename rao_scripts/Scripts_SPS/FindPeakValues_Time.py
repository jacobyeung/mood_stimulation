...
def find_peak_values_time(num_pulses, ERP, norm_evoked_z, test_samples, p, pulse_after):
    ...
    for i_channel in range(num_channels):
        for i_pulse in range(num_pulses):
            apeak_bef = np.abs(norm_evoked_z[Pulse_Before][i_pulse, i_channel, test_samples])
            ...
            apeak_aft = np.abs(norm_evoked_z[pulse_after][i_pulse, i_channel, test_samples])
            ...
            percent_change[i_pulse, i_channel] = ((peak_values_after[i_pulse, i_channel] - peak_values_before[i_pulse, i_channel]) / peak_values_before[i_pulse, i_channel]) * 100
    
    return peak_values_before, peak_values_after, peak_times_before, peak_times_after, percent_change
