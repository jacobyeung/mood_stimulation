function [filteredData,cfs] = KSedits_processingHilbertTransform_filterbankGUI(ecog,Fs,freqRange)
%{

From Chang Speech lab 
...
% filteredData.data = NaN(size(ecog,1),T,npbs);
filteredData.envData = NaN(size(ecog,1),T,npbs);
...
%         filteredData.data(iChan,:,f) = hilbdata;
        filteredData.envData(iChan,:,f) = abs(hilbdata);
